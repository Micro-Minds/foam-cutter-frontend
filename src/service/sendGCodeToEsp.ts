    ...
    return new Promise<void>((resolve, reject) => {
        ...
        socket.onopen = async () => {
            ...
            try {
                ...
                await new Promise(res => setTimeout(res, 200));
                socket.send("$21=0");   // 4. Enable hard limits
                await new Promise(res => setTimeout(res, 200));
...
export async function sendGcodeToESP(gcode: string): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        // Setup + G-code combined queue
        const commandQueue = ["$X", "$23=2", "$22=1", "$21=1", "$H","$100=55.556","$101=55.556","$$", ...lines];
        let currentIndex = -1;
        ...
        function sendNextCommand() {          //sends only one line at a time, and then waits for "ok" from GRBL before moving to the next
            ...
            if (cmd) {
                ...
                socket.send(cmd);// send  gcode line
...
export async function sendGcodeToESP(gcode: string): Promise<void> {
    const espUrl = "ws://192.168.8.150:81"; // Updated ESP32 WebSocket address
; // ESP32 WebSocket address
    const lines = gcode
        .split("\n")
        .map(line => line.trim())
        .filter(line => line !== "");
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        socket.onmessage = (event) => {
            ...
            // Sometimes status or other messages — ignore or log
                ...
                console.log("ℹ Message from GRBL:", response);
            }
        };
